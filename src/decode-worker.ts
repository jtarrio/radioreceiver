...
class Decoder {
   ...
   */
  process(buffer: ArrayBuffer, inStereo: boolean, freqOffset: number, opt_data?: object) {
    ...
    (self as DedicatedWorkerGlobalScope).postMessage([out.left, out.right, data], [out.left, out.right]);
  }
   ...
   */
  setMode(mode: { modulation: string, bandwidth: number, maxF: number }) {
