...
class RTL2832U {
  ...
  constructor(private com: RtlCom) {
    this.tuner = new class implements Tuner {
      async setFrequency(freq: number): Promise<number> { return 0; }
      async setAutoGain() { }
  ...
  private async _init() {
    ...
    await this.com.setUsbReg(0x2000, 0b00001001, 1);
    // USB_EPA_MAXPKT [10:0] Max packet size = 0x200 bytes
    await this.com.setUsbReg(0x2158, 0x0200, 2);
    ...
    await this.com.setSysReg(0x300b, 0b00100010);
    // DEMOD_CTL [3] ADC_Q enable [5] Release reset [6] ADC_I enable [7] PLL enable
    await this.com.setSysReg(0x3000, 0b11101000);
    this._resetDemodulator();
    ...
    await this.com.setDemodReg(1, 0x16, 0x00, 1);
    await this.com.setDemodReg(1, 0x17, 0x00, 1);
    await this.com.setDemodReg(1, 0x18, 0x00, 1);
    // IF frequency registers [21:0] set to 0
    await this.com.setDemodReg(1, 0x19, 0x00, 1);
    await this.com.setDemodReg(1, 0x1a, 0x00, 1);
    await this.com.setDemodReg(1, 0x1b, 0x00, 1);
    // LPF coefficients
    await this.com.setDemodReg(1, 0x1c, 0xca, 1);
    await this.com.setDemodReg(1, 0x1d, 0xdc, 1);
    await this.com.setDemodReg(1, 0x1e, 0xd7, 1);
    await this.com.setDemodReg(1, 0x1f, 0xd8, 1);
    await this.com.setDemodReg(1, 0x20, 0xe0, 1);
    await this.com.setDemodReg(1, 0x21, 0xf2, 1);
    await this.com.setDemodReg(1, 0x22, 0x0e, 1);
    await this.com.setDemodReg(1, 0x23, 0x35, 1);
    await this.com.setDemodReg(1, 0x24, 0x06, 1);
    await this.com.setDemodReg(1, 0x25, 0x50, 1);
    await this.com.setDemodReg(1, 0x26, 0x9c, 1);
    await this.com.setDemodReg(1, 0x27, 0x0d, 1);
    await this.com.setDemodReg(1, 0x28, 0x71, 1);
    await this.com.setDemodReg(1, 0x29, 0x11, 1);
    await this.com.setDemodReg(1, 0x2a, 0x14, 1);
    await this.com.setDemodReg(1, 0x2b, 0x71, 1);
    await this.com.setDemodReg(1, 0x2c, 0x74, 1);
    await this.com.setDemodReg(1, 0x2d, 0x19, 1);
    await this.com.setDemodReg(1, 0x2e, 0x41, 1);
    await this.com.setDemodReg(1, 0x2f, 0xa5, 1);
    // ? claimed to be enable SDR, [5] disable DAGC
    await this.com.setDemodReg(0, 0x19, 0b00000101, 1);
    // ? claimed to be initialize finite-state machine
    await this.com.setDemodReg(1, 0x93, 0b11110000, 1);
    await this.com.setDemodReg(1, 0x94, 0b00001111, 1);
    // [0] disable DAGC
    await this.com.setDemodReg(1, 0x11, 0b00000000, 1);
    // [4:1] set AGC loop gain to 0
    await this.com.setDemodReg(1, 0x04, 0b00000000, 1);
    // [5] pass error packets [6] reject matched PID
    await this.com.setDemodReg(0, 0x61, 0b01100000, 1);
    // [5:4] default ADC_I, ADC_Q datapath
    await this.com.setDemodReg(0, 0x06, 0b10000000, 1);
    // [0] enable zero-IF input
    await this.com.setDemodReg(1, 0xb1, 0b00011011, 1);
    // ? claimed to be disable output on TP_CK0
    await this.com.setDemodReg(0, 0x0d, 0b10000011, 1);

    this.tuner = await this._FindTuner();
  ...
  async setFrequencyCorrection(ppm: number) {
    ...
    await this.com.setDemodReg(1, 0x3e, (ppmOffset >> 8) & 0x3f, 1);
    await this.com.setDemodReg(1, 0x3f, ppmOffset & 0xff, 1);
    this.tuner.setXtalFrequency(this._getXtalFrequency());
  ...
  private async _setIntermediateFrequency(freq: number) {
    let multiplier = -1 * Math.floor(freq * (1 << 22) / this._getXtalFrequency());
  ...
  private async _FindTuner(): Promise<Tuner> {
    await this.com.openI2C();
    ...
    this._setIntermediateFrequency(R820T.IF_FREQ);
    // [0] inverted spectrum
    await this.com.setDemodReg(1, 0x15, 0b00000001, 1);
    let tuner = await R820T.init(this.com, this._getXtalFrequency());
    await this.com.closeI2C();
    return tuner;
  }
}
