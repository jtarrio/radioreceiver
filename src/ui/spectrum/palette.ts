...
export function CubeHelix(start: number, rotations: number, hue: number, gamma: number): Palette {
    ...
    for (let i = 0; i < colors; ++i) {
        ...
        const red = gammaFraction + amplitude * (-0.14861 * cosine + 1.78277 * sine);
        const green = gammaFraction + amplitude * (-0.29227 * cosine - 0.90649 * sine);
