...
class RadioController {
    ...
    async statePlaying() {
        ...
        let data = await this.tuner.readSamples(RadioController.SAMPLES_PER_BUF);
        --this.requestingBlocks;
        if (this.state.state == STATE.PLAYING) {
            if (this.playingBlocks <= 2) {
                ++this.playingBlocks;
                this.decoder.postMessage([0, data, this.stereoEnabled, this.actualFrequency - this.frequency], [data]);
            }
        }
        this.processState();
    }
    ...
    async stateScanning() {
        ...
        if (this.state.substate == SUBSTATE.TUNING) {
            ...
            if (this.state.state == STATE.SCANNING) {
                ++this.playingBlocks;
                this.decoder.postMessage([0, data, this.stereoEnabled, this.actualFrequency - this.frequency, scanData], [data]);
            }
        }
        this.processState();
    }
